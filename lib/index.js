        ...
        var makeRequest = protocol.request(proxyURL.href, proxyOptions, proxyResponse => {
            ...
            proxyResponse.on('data', data => rawData.push(data)).on('end', () => {
                ...
                proxify.url = url => {
                 ...
                 proxify.js = buffer => buffer.toString().replace(/(,| |=|\()document.location(,| |=|\)|\.)/gi, str => { return str.replace('.location', `.Unblocker999Location`); })
                     .replace(/(,| |=|\()window.location(,| |=|\)|\.)/gi, str => { return str.replace('.location', `.Unblocker999Location`); })
                     .replace(/(,| |=|\()location(,| |=|\)|\.)/gi, str => { return str.replace('location', `Unblocker999Location`); });
