// Unblocker999 javascript object rewriter.
// Rewrites functions that makes HTTP or Websocket requests, and DOM element selectors & creators.

// Unblocker999 configurations retrieved from attribute.

var Unblocker999 = JSON.parse(atob(document.currentScript.getAttribute('data-config')));
Unblocker999.url = new URL(Unblocker999.url)

// "document.location" and "window.location" are rewritten server-side and replaced with this object.
window.Unblocker999Location = new Proxy({}, {
    set(obj, prop, value) {

        if (prop == 'assign' || prop == 'reload' || prop == 'replace' || prop == 'toString') return;

        console.log(proxify.url(Unblocker999.url.href.replace(Unblocker999.url[prop], value)));

        console.log((Unblocker999.url.href.replace(Unblocker999.url[prop], value)));


        return location[prop] = proxify.url(Unblocker999.url.href.replace(Unblocker999.url[prop], value));
    },
    get(obj, prop) {
        // Had to be done in order to fix Discord.
        if (Unblocker999.url.origin == atob('aHR0cHM6Ly9kaXNjb3JkLmNvbQ==') && Unblocker999.url.pathname == '/app') return window.location[prop];

        if (prop == 'assign' || prop == 'reload' || prop == 'replace' || prop == 'toString') return {
            assign: arg => window.location.assign(proxify.url(arg)),
            replace: arg => window.location.replace(proxify.url(arg)),
            reload: () => window.location.reload(),
            toString: () => { return Unblocker999.url.href }
        }[prop];
        else return Unblocker999.url[prop];
    }    
});

window.document.Unblocker999Location = window.Unblocker999Location;

Object.defineProperty(document, 'domain', {
    get() {
        return Unblocker999.url.hostname;
    },
    set(value) {
        return value;
    }
...
// Any Unblocker999 function that rewrites request URLs go under this object.
var proxify = {
    url: (url, type) => {
        ...
        switch(type) {
            case true:
                proxified = atob(url.replace(Unblocker999.prefix, '').split('_').slice(1).splice(0, 1).join()) + url.split('_').slice(2).join('_');
            break;

            default:

                if (url.match(/^(#|about:|data:|blob:|mailto:|javascript:|{|\*)/) || url.startsWith(Unblocker999.prefix) || url.startsWith(window.location.origin + Unblocker999.prefix)) return url;

                if (url.startsWith(window.location.origin + '/') && !url.startsWith(window.location.origin + Unblocker999.prefix)) url = '/' + url.split('/').splice(3).join('/');

                if (url.startsWith('//')) url = 'http:' + url;
                if (url.startsWith('/') && !url.startsWith(Unblocker999.prefix)) url = Unblocker999.url.origin + url;

                if (url.startsWith('https://') || url.startsWith('http://')) url = new URL(url);
                else url = new URL(Unblocker999.url.href.split('/').slice(0, -1).join('/') + '/' + url);

                proxified = Unblocker999.prefix + '_' + btoa(url.href.split('/').splice(0, 3).join('/')) + '_' + "/" + url.href.split('/').splice(3).join('/');
...
proxify.url_http = url => {
    ...
    else if (Unblocker999.baseURL) {
        if (url.startsWith('/')) return url = proxify.url(Unblocker999.baseURL.split('/').splice(0, 3).join('/') + url);
        else return url = proxify.url(Unblocker999.baseURL.split('/').slice(0, -1).join('/') + '/' + url);
...
window.fetch = function(url, options) {
    ...
    if (url) (url.replace(location.hostname, Unblocker999.url.hostname), url = proxify.url_http(url));
...
window.WebSocket = new Proxy(window.WebSocket, {
    construct(target, args) {
        ...
        args[0] = protocol + location.origin.split('/').splice(2).join('/') + Unblocker999.prefix + '?ws=' + btoa(args[0]) + '&origin=' + btoa(Unblocker999.url.origin);
...
window.History.prototype.pushState = new Proxy(window.History.prototype.pushState, {
    apply(target, thisArg, args) {
        ...
        // Discord support
        if (Unblocker999.url.origin == atob('aHR0cHM6Ly9kaXNjb3JkLmNvbQ==') && args[2] == '/app') {
    ...
    get() {
        var cookie = Object.getOwnPropertyDescriptor(window.Document.prototype, 'cookie').get.call(this),
            ...
            cookie_array.forEach(cookie => {
                ...
                if (Unblocker999.url.hostname.includes(cookie_name.split('@').splice(1).join())) new_cookie.push(cookie_name.split('@').splice(0, 1).join() + '=' + cookie_value);
